...
3. **Atividades e sub-atividades, em qualquer profundidade** — cada atividade pode ter uma ou mais sub-atividades, e uma sub-atividade também pode ter as suas próprias sub-atividades (sem limite de níveis), descrição, responsável, área e imagens anexadas (fotos do local, desenhos, etc.). A listagem fica sempre ordenada por Data/Hora de Início, com cada sub-atividade agrupada e indentada logo abaixo da atividade correspondente, e o sistema impede que uma atividade seja definida como sub-atividade de algo que já é seu próprio descendente.
...
7. **Visão em Tabela e Gantt** — tabela com filtros, ordenação por coluna e exportação para Excel; Gantt com zoom por **semana, dia ou horas** (este último com cabeçalho detalhado hora a hora, útil para atividades curtas ou para acompanhar o andamento no dia), barra de progresso, cores por status e uma barra tracejada mostrando a execução real ao lado da planejada.
